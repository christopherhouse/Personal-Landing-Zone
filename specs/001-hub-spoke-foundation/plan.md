...
- Steady-state idle hub + one-spoke cost target: ≤ $500/month at the chosen minimum SKUs (VpnGw2AZ ≈ $268 — Azure now rejects all non-AZ VPN SKUs at create time with `NonAzSkusNotAllowedForVPNGateway`; see research.md R-005; DNS Private Resolver inbound endpoint ≈ $130; Log Analytics workspace + validation VM + storage PE ≈ $25–$50; per-spoke NAT Gateway ≈ $33 — required because Azure retired default outbound for new VNets on 2025-09-30 and the AADSSHLoginForLinux extension must reach public package registries during install). Constitution Principle II's "minimum SKU" intent is honored within the constraints Azure exposes. Recorded in `docs/cost-baseline.md` and reviewed on every PR that adds an always-on resource (SC-007).
...
|---|---|---|
| Native `azurerm_public_ip` (for the VPN gateway PIP) instead of `Azure/avm-res-network-publicipaddress` (Constitution Principle I, escape hatch (c)) | Azure auto-populates `ip_tags = { FirstPartyUsage = "/Unprivileged" }` post-create. The AVM module v0.2.1 doesn't expose `ip_tags` as an input and doesn't pass `lifecycle.ignore_changes` through to its internal resource. The resulting drift forces destroy+create on every plan, which would rotate the gateway's public IP every apply and invalidate every downloaded Azure VPN Client profile. Native resource with `lifecycle { ignore_changes = [ip_tags] }` fixes it. | Filing an upstream AVM PR is the right long-term answer but blocks delivery indefinitely. Trying to set `ip_tags` to a matching map at the AVM caller wouldn't work because the AVM module ignores caller-supplied tags-of-IP and Azure sets the value asynchronously. |
| Native `azurerm_virtual_network_gateway` instead of an AVM module (Constitution Principle I, escape hatch (c)) | The dedicated AVM pattern module `Azure/avm-ptn-vnetgateway` was archived in October 2025. Its successor `Azure/avm-ptn-alz-connectivity-hub-and-spoke-vnet` (v0.17.1, GA) is opinionated for Azure Landing Zone CAF alignment: it forces Firewall, Bastion, DDoS Standard, and a fixed connectivity-subscription shape. There is no resource-scope AVM (`avm-res-network-virtualnetworkgateway`) at GA yet; it is still in "proposed" status. | Composing the resource-scope AVMs (VNet, public IP, NSG, DNS zones, resolver, workspace, VM, storage) plus the native VPN gateway is the only shape that simultaneously honors Principle II (minimum SKU, no Firewall, no Bastion, no DDoS Standard) and Principle V (pin a stable version we can audit). The ALZ pattern would directly violate Principle II. A "wait for resource-scope AVM" approach blocks delivery indefinitely. This native fallback will be re-evaluated whenever the AVM index publishes `avm-res-network-virtualnetworkgateway` at GA. |
